...
def extract_features(
    ...
    if use_mask:
        ...
        features_fix = MINDSSC(fixed_r.to(device), mind_r, mind_d, device=device.type).to(dtype)
        features_mov = MINDSSC(moving_r.to(device), mind_r, mind_d, device=device.type).to(dtype)
    else:
        img_fixed = img_fixed.unsqueeze(0).unsqueeze(0)
        img_moving = img_moving.unsqueeze(0).unsqueeze(0)
        features_fix = MINDSSC(img_fixed.to(device), mind_r, mind_d, device=device.type).to(dtype)
        features_mov = MINDSSC(img_moving.to(device), mind_r, mind_d, device=device.type).to(dtype)
